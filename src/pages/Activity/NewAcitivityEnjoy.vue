<template>
	<div class="pageWidth">
		...
		<EnjoyDiscount v-if="0"></EnjoyDiscount>
		<div class="introBtn">
						...
						<div class="sidebar">
							...
							<div class="menuContent" ref="menu">
								...
								<div v-for="(item,index) in menuList" :key="item.id" :class="item.class" @click="choiceMenu(item.id)" :id=item.id>
									<div class="my-wrap">
										<div @click.stop='downCircle(index)' class="downCircle" v-if="index===0?false:true"><i class="circle iconfont icon-jiantouarrow505"></i></div>
										<div @click.stop='topCircle(index)' class="topCircle" v-if="index===0?false:true"><i class="circle iconfont icon-jiantouarrow499"></i></div>
									...
									<div class="isFall" v-if="isFall&&item.id===3?true:false" ref="isFall">
										<div v-for="(item,index) in formFalls.waterfallDetails" :key="item.id" :class="item.class">
											...
											<el-row :gutter="0">
												<el-col :span="7">
													<div class="btb-wrap">
														<div @click.stop='topFall(index)' class="fallCircle"><i class="circle iconfont icon-jiantouarrow499"></i></div>
														<div @click.stop='downFall(index)' class="fallCircle"><i class="circle iconfont icon-jiantouarrow505"></i></div>
														<div @click.stop='delFall(item.id)' class="fallCircle"><i class="circle iconfont icon-icon--"></i></div>
												...
												<el-col :span="13">
													<div><span class="fallFont">按钮颜色：</span><input class="fallColor" placeholder="请选择" v-model="item.color"></div>
													<div><span class="fallFont">按钮名字：</span><input class="fallColor" placeholder="请输入" v-model="item.buttonName"></div>
												</el-col>
												<el-col :span="3">
				...
				<el-col :span="14" :offset="1" v-if="isHeader">
					<div class="rightForm">
						<div class="headerTitle">
							...
							<el-switch :width=100 @change=isUseChange v-model='headerIsUse' on-text="启用" off-text="停用" on-color="#DB5050" off-color="#4F4D4D">
							</el-switch>
						</div>
						<hr class="lineWeight">
						<el-form ref="formData" :rules="rules" :model="form" label-width="100px" class="formData">
							...
							<el-form-item label="是否登录：" prop="headerNeedLogin">
								<el-switch :width=60 v-model='headerNeedLogin' on-text="登录" off-text="关闭" on-color="#DB5050" off-color="#4F4D4D">
								</el-switch>
							</el-form-item>
							...
							<el-form-item label="有效日期：" prop="beginDate">
								<el-date-picker v-model="form.beginDate" type="datetime" @change="beginDateChange" placeholder="选择生效时间">
								</el-date-picker>
								-
								<el-date-picker v-model="form.endDate" type="datetime" @change="endDateChange" placeholder="选择失效时间">
								</el-date-picker>
							</el-form-item>
				...
				<el-col v-if='isCarousel' :span="14" :offset="1">
					<div class="rightForm">
						<div class="headerTitle">
							...
							<el-switch :width=100 v-model="carouselIsUse" @change=isUseChange on-text="启用" off-text="停用" on-color="#DB5050" off-color="#4F4D4D">
							</el-switch>
						</div>
						<hr class="lineWeight">
						<el-form ref="formCarousel" :rules="rules" :model="formCarousel" label-width="120px" class="formData">
							...
							<el-form-item label="是否显示：" prop="carouselIsShow">
								<el-switch :width=60 v-model="carouselIsShow" on-text="显示" off-text="隐藏" on-color="#DB5050" off-color="#4F4D4D">
								</el-switch>
							</el-form-item>
							<el-form-item label="是否登录：" prop="carouselNeedLogin">
								<el-switch :width=60 v-model="carouselNeedLogin" on-text="登录" off-text="关闭" on-color="#DB5050" off-color="#4F4D4D">
								</el-switch>
							</el-form-item>
							...
							<div class="carousel" v-for="(item,index) in formCarousel.carouselDetails" :key="index">
								...
								<el-row :gutter="0">
									...
									<el-col :span="4">
										...
										<el-upload :ref="item.id" class="avatar-uploader" action='http://jjdcjavaweb.oss-cn-shanghai.aliyuncs.com' :data="Token" :show-file-list="false" :on-success="handleAvatar" :before-upload="beforeAvatarUpload">
										...
										<el-form :rules="rules" label-width="100px">
											...
											<el-form-item label="类型：" prop="type">
												<el-select v-model="item.type" placeholder="请选择类型" @change="enjoyTypeChange">
													<el-option v-for="(option,index) in enjoyType" :key="index" :label="option.label" :value="option.value">
													</el-option>
												</el-select>
											</el-form-item>
											<el-form-item label="跳转类型：" v-show="(item.type === 0|| item.type === 1?true:false)" prop="actionType">
												<el-select v-model="item.actionType" placeholder="请选择跳转类型">
													<el-option v-for="(option,index) in (item.type===0?actionType:actionType2)" :key="index" :label="option.label" :value="option.value">
													</el-option>
												</el-select>
											</el-form-item>
											...
											<el-form-item label="链接：" v-show="item.type === 2?true:false">
												<el-select v-model="item.actionUrl" placeholder="请选择链接">
													<el-option v-for="(option,index) in enjoyUrl" :key="index" :label="option.label" :value="option.value">
													</el-option>
												</el-select>
											</el-form-item>
											<el-form-item label="展示日期：" prop="beginDate">
												<el-date-picker v-model="item.beginDate" type="datetime">
												</el-date-picker>
												-
											...
											<el-form-item label="平台：">
												<el-select v-model="item.os" placeholder="请选择平台">
													<el-option v-for="(option,index) in showType" :key="index" :label="option.label" :value="option.value">
													</el-option>
												</el-select>
											</el-form-item>
				...
				<el-col v-if='isIcon' :span="14" :offset="1">
					<div class="rightForm">
						<div class="headerTitle">
							...
							<el-switch :width=100 v-model="iconDataList.isUse" @change=isUseChange  on-text="启用" off-text="停用" on-color="#DB5050" off-color="#4F4D4D">
							</el-switch>
						</div>
						<hr class="lineWeight">
						<el-form ref="formIcon" :rules="rules" :model="formIcon" label-width="110px" class="formData">
							...
							<el-form-item label="是否显示：">
								<el-switch :width=60 v-model="iconDataList.isShow" on-text="开启" off-text="关闭"  on-color="#DB5050" off-color="#4F4D4D">
								</el-switch>
							</el-form-item>
							<el-form-item label="是否登录：">
								<el-switch :width=60 v-model="iconDataList.isLogin" on-text="开启" off-text="关闭" on-color="#DB5050" off-color="#4F4D4D">
								</el-switch>
							</el-form-item>

							<el-form-item label="模块宽高：">
								<el-input v-model="iconDataList.width" placeholder="宽" class="miWidth"></el-input>
								x
								<el-input v-model="iconDataList.height" placeholder="高" class="miWidth"></el-input>
							</el-form-item>
										...
										<el-form :label-position="'right'" :rules="rules" label-width="100px">
											...
											<el-form-item label="生效日期" prop="beginDate">
												<template slot-scope="scope">
													<el-date-picker type="datetime" value-format="yyyy-MM-dd HH:mm:ss" v-model="item.startTime" @change="changeTimeFn(scope.row)" placeholder="选择日期时间">
													</el-date-picker>
												</template>
											</el-form-item>
										</el-form>
										...
										<el-form :ref=index :rules="rules" label-width="100px">
											...
											<el-form-item label="指向列表：" v-show="isActionType">
												<el-select v-model="item.actionType" placeholder="请选择" @change="actionTypeChange">
													<el-option v-for="(option,index) in actionType" :key="index" :label="option.label" :value="option.value">
													</el-option>
				...
				<el-col :span="14" :offset="1" v-if="isFalls">
					<div class="rightForm">
						<div class="headerTitle">
							...
							<el-switch :width=100 @change=isUseChange v-model='fallIsUse' on-text="启用" off-text="停用" on-color="#DB5050" off-color="#4F4D4D">
							</el-switch>
						</div>
						<hr class="lineWeight">
						<el-form ref="formData" :rules="rules" :model="formFalls" label-width="100px" class="formData">
							...
							<el-form-item label="是否显示：" prop="headerNeedLogin">
								<el-switch :width=60 v-model='fallIsShow' on-text="登录" off-text="关闭" on-color="#DB5050" off-color="#4F4D4D">
								</el-switch>
							</el-form-item>
							<el-form-item label="是否登录：" prop="headerNeedLogin">
								<el-switch :width=60 v-model='fallIsLogin' on-text="登录" off-text="关闭" on-color="#DB5050" off-color="#4F4D4D">
								</el-switch>
							</el-form-item>
							<el-form-item label="模块宽高：">
		...
		<el-dialog title="城市" size="tiny" :visible.sync="cityVisible" center>
			...
			<el-tree :data="select" default-expand-all node-key="id" :default-checked-keys="checkedCity" ref="tree" class="treeWidth" accordion :filter-node-method="filterNode" @node-click="handleNode" :props="defaultProps">
			</el-tree>
...
<script>
	...
	import EnjoyDiscount from './components/EnjoyDiscount'
	...
	export default {
		...
		methods: {
			...
			getRanks() {
				...
				this.$ajax.get(`${baseUrl.newEnjoyUrl}/jjEnjoy/getRank`, {
						params: {
							cityId: this.cityId
						}
					})
			...
			//页头
			getHeader() {
				this.$ajax.get(`${baseUrl.newEnjoyUrl}/jjEnjoy/title/form`, {
						params: {
							cityId: this.cityId
						}
					})
			...
			beforeAvatarUpload(file) {
				return new Promise((resolve) => {
					this.$ajax.get(`${baseUrl.mainUrl}/electric/ossutil/interface/policy`, {
							params: {
								user_dir: 'newActivityEnjoy'
							}
						})
						.then((res) => {
							this.Token = res.data
			...
			getEnjoyType() {
				this.$ajax.get(`${baseUrl.mainUrl}/sys/dictutils/interface/getDictList`, {
						params: {
							type: 'enjoy_type'
						}
					})
			...
			getCarousel() {
				this.$ajax.get(`${baseUrl.newEnjoyUrl}/jjEnjoy/carousel/form`, {
						params: {
							cityId: this.cityId
						}
					})
			...
			delFall(id) {
				this.$ajax.post(`${baseUrl.newEnjoyUrl2}/jjEnjoy/waterFall/deleteWaterFall`, {
						id: id
					})
					.then((res) => {
						this.$message.success(res.data.msg)
			...
			getIcon() {
				this.$ajax.get(`${baseUrl.newEnjoyUrl3}/jjEnjoy/icon/form`, {
					...
					if(res.status == 200) {
						...
						if (this.iconDataList.isUse == 1){
							this.$set(this.iconDataList,"isUse",true)
						} else {
							this.$set(this.iconDataList,"isUse",false)
						}
						if (this.iconDataList.isLogin == 1){
							this.$set(this.iconDataList,"isLogin",true)
						} else {
							this.$set(this.iconDataList,"isLogin",false)
						}
						if (this.iconDataList.isShow == 1){
							this.$set(this.iconDataList,"isShow",true)
						} else {
							this.$set(this.iconDataList,"isShow",false)
						}
            
            if (this.iconDataList.ratio.indexOf("x") != -1 ){
            	const arr = this.iconDataList.ratio.split("x")
            	this.$set(this.iconDataList,'width',arr[0])
            	this.$set(this.iconDataList,'height',arr[1])
            }
			...
			saveIcon() {
				let copyObj = deepcopy(this.iconDataList)
				console.log(copyObj)
				if (copyObj.isUse == true){
							this.$set(copyObj,"isUse",1)
						} else {
							this.$set(copyObj,"isUse",0)
						}
						if (copyObj.isLogin == true){
							this.$set(copyObj,"isLogin",1)
						} else {
							this.$set(copyObj,"isLogin",0)
						}
						if (copyObj.isShow == true){
							this.$set(copyObj,"isShow",1)
						} else {
							this.$set(copyObj,"isShow",0)
						}
            
            if (copyObj.width&&copyObj.height){
            	this.$set(copyObj,'ratio',copyObj.width +"x"+copyObj.height)
            }
				console.log(this.iconDataList,copyObj, "*************")
				
				this.$ajax.post(`${baseUrl.newEnjoyUrl3}/jjEnjoy/icon/saveIconDetails`, {
					iconDetails: JSON.stringify(copyObj)
				}).then((res) => {
					//console.log(res)
				}).catch(err => {
