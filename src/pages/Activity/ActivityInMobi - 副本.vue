...
// 合并请求开始
        function getUserAccount () {
          return this.$ajax.get('sys/dictutils/interface/getDictList', {params: {type: 'inmobi_display_type'}})
        }

        function getUserPermissions () {
          return this.$ajax.get('sys/dictutils/interface/getDictList', {params: {type: 'inmobi_type'}})
        }

        this.$ajax.all([getUserAccount(), getUserPermissions()])
          .then(this.$ajax.spread(function (disObjRes, typeObjRes) {
            for (var i = 0; i < disObjRes.data.length; i++) {
              this.disObj[disObjRes.data[i].value] = disObjRes.data[i].label
            }
            for (var j = 0; j < typeObjRes.data.length; j++) {
              this.typeObj[typeObjRes.data[j].value] = typeObjRes.data[j].label
            }
          }))
          // 合并请求结束
