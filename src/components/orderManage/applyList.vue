...
// 	this.$refs.search.checkOrderNodeFn()
 ...
 created() {
 	if (JSON.parse(localStorage.getItem('myPageSize'))) { 	
 		this.pageSize = JSON.parse(localStorage.getItem('myPageSize')).W_ApplyList?JSON.parse(localStorage.getItem('myPageSize')).W_ApplyList:10
 		console.log(JSON.parse(localStorage.getItem('myPageSize')).W_ApplyList)
 	} else {
 		let obj = {}
  ...
  methods: {
  	queryApplyOrderInfoFn() {
  		...
  		const pararms = {
			...
			orderStatus: [1],//订单状态：1申请中,2审批中,3还款中,4已结清,5拒绝,6线上筹资中,7满标,8满标以放款,9流标,10退件
